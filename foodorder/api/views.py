...
def get_invoice(request, order_number):
    ...
    orders = Order.objects.filter(order_number=order_number, is_order_placed=True).select_related('food')
    # Delivery address fetch karna
    address = OrderAddress.objects.get(order_number=order_number)
    
    grand_total = 0
    order_data = []
    
    # Pratyek item ke liye total price aur grand total calculate karna
    for order in orders:
    ...
    # Return render loop ke bahar hona chahiye (Fix indentation bug from video)
...
def update_user_profile(request, user_id):
    user = User.objects.get(id=user_id)
    # partial=True ka matlab hai ki hum sirf kuch fields update kar rahe hain
...
def change_password(request, user_id):
    ...
    try:
        ...
        # Naya password hash karke save karna
...
def view_order_detail(request, order_number):
    try:
        # select_related ka upyog user info ke liye
...
def update_order_status(request):
    ...
    try:
        # Order ki main address detail fetch karein
        address = OrderAddress.objects.get(order_number=order_number)
        
        # Tracking record create karne ke liye Order object fetch karein
        order = Order.objects.filter(order_number=order_number).first()
        
        if not order:
            return Response({'error': 'Order not found'}, status=404)

        # FoodTracking table mein history save karein
        ...
        # Main OrderAddress table mein final status update karein
...
def search_orders(request):
    query = request.GET.get('q', '')
    if query:
        # __icontains ka use case-insensitive partial match ke liye kiya gaya hai
...
def category_detail(request, id):
    try:
        # ID ke basis par category fetch karna
        category = Category.objects.get(id=id)
    except Category.DoesNotExist:
        return Response({'error': 'Category Not Found'}, status=404)

    # 1. GET Request: Single category ka data fetch karne ke liye (Edit form mein dikhane ke liye)
    if request.method == 'GET':
        serializer = CategorySerializers(category)
        return Response(serializer.data)

    # 2. PUT Request: Category ko update karne ke liye
    ...
    # 3. DELETE Request: Category ko delete karne ke liye
...
def delete_food(request, id):
    try:
        # ID ke basis par food item fetch karna
        food = Food.objects.get(id=id)
        food.delete() # Database se delete karna
...
def edit_food(request, id):
    ...
    # GET Request: Form pre-fill karne ke liye data bhejra hai
    if request.method == 'GET':
        serializer = FoodSerializers(food)
        return Response(serializer.data)
    ...
    # PUT Request: Data update karne ke liye
        ...
        data = request.data.copy() # Data ki copy banayi taaki changes kar sakein

        # Agar nayi image nahi aayi hai, toh purani wali hi rakho
        if 'image' not in request.FILES:
            data['image'] = food.image

        # is_available string ko boolean mein convert karna
...
def edit_food(request, id):
    ...
    if request.method == 'GET':
        ...
        # String "true"/"false" ko boolean mein convert karna
        if 'is_available' in data:
            data['is_available'] = data['is_available'].lower() == 'true'
...
# Helper function to calculate total sales based on start date
def get_sales_total(start_date):
    # Convert date to timezone-aware datetime to avoid naive datetime warning
    aware_start = make_aware(datetime.combine(start_date, datetime.min.time()))
    
    # Filter paid orders order numbers from PaymentDetail table
    paid_order_numbers = PaymentDetail.objects.filter(payment_date__gte=aware_start).values_list('order_number', flat=True)
    
    # Calculate Sum of (Quantity * Food Price) for those specific orders
...
def dashboard_metrics(request):
    # Boundary dates calculate karein
    today_date = now().date()
    start_of_week = today_date - timedelta(days=today_date.weekday()) # Monday of current week
    ...
    data = {
        ...
        # Sales metrics calculation call
        "today_sales": get_sales_total(today_date),
        "week_sales": get_sales_total(start_of_week),
        "month_sales": get_sales_total(start_of_month),
        "year_sales": get_sales_total(start_of_year),
    }
...
def monthly_sales_summary(request):
    # स्टेप 1: प्रत्येक ऑर्डर का कुल मूल्य निकालें
    orders = (
    ...
    # स्टेप 2: महीनों के अनुसार डेटा ग्रुप करें
    addresses = (
...
def weekly_user_registrations(request):
    # 'reg_date' को सप्ताह के आधार पर ग्रुप करें और यूजर्स की संख्या काउंट करें
    data = (
        User.objects
    ...
    # डेटा को चार्ट के अनुकूल फॉर्मेट में बदलें
    result = [
...
def cancel_order(request, order_number):
    ...
    try:
        ...
        # 2. Tracking table mein entry karein
...
def review_detail(request, id):
    ...
    # Review delete karne ke liye
    if request.method == 'DELETE':
        review.delete()
        return Response({"message": "Review deleted"}, status=200)
    
    # Review update karne ke liye
...
def food_rating_summary(request, food_id):
    # Us food item ke sabhi reviews nikalna
    reviews = Review.objects.filter(food_id=food_id)
    
    # 1. Per rating count (5 star kitne hain, 4 star kitne...)
    # values('rating') group by karta hai aur annotate count karta hai
    rating_summary = reviews.values('rating').annotate(count=Count('rating')).order_by('-rating')
    
    # Dictionary comprehension se breakdown list banana
    breakdown = {entry['rating']: entry['count'] for entry in rating_summary}
    
    # 2. Average rating nikalna aur round karna
